...
- 强化上下文归因规则：明确 assistant 消息与工具结果属于千惠自己已完成的动作，只有带真实用户名的 user 消息才算新输入；`reply()` 成功后收束本轮，避免追着自己的回复连续发言。

## 语音

- 语音合成配置不可用时（provider 不受支持、参考音频未配置）降级为文本发送，不再因配置错误丢掉整条回复；真正的合成故障仍受 `tts_fallback_to_text` 控制。

## 私密模式

- 台词语音的等待时间改为按后端 `tts_timeout` 推导，避免合成尚未结束就停止轮询；连续查询失败不再立即放弃，并在超时或失败时明确提示文字回复已保留。
- 补充私密控制台 API 与语音配置降级行为的回归测试。

# [1.1.0] - 2026-7-23

## 项目品牌

- 项目主品牌由 MaiBot/麦麦统一调整为 Project Chie/千惠，覆盖启动信息、WebUI、文档、部署脚本和包元数据；保留 MaiBot 上游署名及旧数据、插件 SDK 等兼容标识。
